...
class PingApp:
    ...
    def write_to_sheet(self, domain, packets_sent, packet_loss, latency):
        try:
            sheet = workbook.worksheet("Trang tính1")  # Đảm bảo tên bảng tính là "Sheet1"
            row = [time.strftime("%Y-%m-%d %H:%M:%S"), domain, packets_sent, packet_loss, latency]
